...
def construct_pipeline():
    ...
    if not os.path.exists(detection["labels"]):
        ...
        sys.exit(1)
    pipeline_str = """
qtiqmmfsrc name=qmmf ! video/x-raw,format=NV12,width=1920,height=1080,framerate=30/1 ! tee name=split
split. ! queue ! qtimetamux name=metamux ! queue ! qtivoverlay ! queue ! v4l2h264enc ! h264parse ! mpegtsmux ! udpsink host=192.168.0.44 port=5004
...
qtimlvdetection threshold=60.0 results=5 module=yolov8 labels=/etc/labels/coco_labels.txt constants="YoloV8,q-offsets=<33.0,0.0,0.0>,q-scales=<3.243085384,0.0037704327,1.0>;" ! queue ! tee name=aitee
aitee. ! text/x-raw ! queue ! metamux.
aitee. ! text/x-raw ! queue ! appsink name=appsink emit-signals=true drop=true max-buffers=1 sync=false
    """
